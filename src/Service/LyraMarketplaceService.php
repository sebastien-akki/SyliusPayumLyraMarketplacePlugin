...
class LyraMarketplaceService
{
    ...
    {
        try {
            $this->ordersApi->ordersValidate($uuid);
        } catch (Exception $e) {
    ...
    private function processOrder(Order $order, ?string $returnUrl, bool $update = false, bool $forceNew = false): void
    {
        ...
        if (!$update) {
            ...
            $reference = sprintf('%s_%s', $order->getId(), $microtime);
        ...
        //On passe en validation manuelle pour pouvoir ne pas valider la commande côté Lyra tant que Sylius n'a pas fini son process complet de commande
        $data->setAwaitingValidation(true);
